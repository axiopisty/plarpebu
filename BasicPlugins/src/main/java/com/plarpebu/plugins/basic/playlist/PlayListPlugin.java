...
import com.plarpebu.common.PlayerController;
 ...
 * @author kmschmidt
 */
public class PlayListPlugin extends FramePlugin implements BasicPlayerListener, ActionListener,
         MouseListener, MouseMotionListener, DropTargetListener, KeyListener
{
	...
	public PlayListPlugin()
	{
		...
		buttonPrev = SwingUtils.addButton(controlPanel, "Prev", "Prev",
		         "/icons/little/littlePrev.gif");
		buttonPrev.addActionListener(this);

		buttonPlay = SwingUtils.addButton(controlPanel, "Play", "Play",
		         "/icons/little/littlePlay.gif");
		buttonPlay.addActionListener(this);

		buttonPause = SwingUtils.addButton(controlPanel, "Pause", "Pause",
		         "/icons/little/littlePause.gif");
		buttonPause.addActionListener(this);

		buttonStop = SwingUtils.addButton(controlPanel, "Stop", "Stop",
		         "/icons/little/littleStop.gif");
		buttonStop.addActionListener(this);

		buttonNext = SwingUtils.addButton(controlPanel, "Next", "Next",
		         "/icons/little/littleNext.gif");
		buttonNext.addActionListener(this);

		buttonAdd = SwingUtils.addButton(controlPanel, "Add File", "Add File",
		         "/icons/little/littleEject.gif");
		...
		menuItem = new JMenuItem("Quick Find");
		menuItem.setActionCommand("quickfind");
	...
	/**
	 * Permet de creer un nouveau Jtoggle bouton en lui associant, une action, un tooltip et une
	 * icone
	 * 
	 * @param p
	 *           composant dans lequel il va etre ajout�
	 * @param name
	 *           son nom pour lui associer une action(actionPerformed)
	 * @param tooltiptext
	 *           le tooltip a afficher
	 * @param imageName
	 *           le chemin d'acc�s a l'icone
	 */
	...
	{
		PlayListPlugin.controller = (PlayerController) newController;
	}
	...
	/**
	 * Find the next file that matches the search text
	 * 
	 * @param searchText
	 */
	...
	/**
	 * Find the prev file that matches the search text
	 * 
	 * @param searchText
	 */
	...
	/**
	 * Cette methode va permettre a l'utilisateur de sauvegarder la playlist actuelle en mode texte
	 * [lisible donc par l'homme ]. Les 3 premieres lignes indiqueront la fonte , taille et style de
	 * la playlist (au niveau ecriture) ensuite les lignes suivantes seront composees du titre des
	 * chansons avec son numero de lecture. Notons que l'interface de dialogue avec les fichiers
	 * conservent le dernier choix (repertoire + fichier) pour pouvoir a une prochaine ouverture de
	 * la boite de dialogue etre directement au dernier endroit selectionne.
	 */
	...
	/**
	 * Cette methode va permettre a l'utilisateur de charger une playlist [lisible donc par l'homme ].
	 * Les 3 premieres lignes indiqueront la fonte , taille et style de la playlist (au niveau
	 * ecriture) ensuite les lignes suivantes seront composees du titre des chansons avec son numero
	 * de lecture. Notons que l'interface de dialogue avec les fichiers conservent le dernier choix
	 * (repertoire + fichier) pour pouvoir a une prochaine ouverture de la boite de dialogue etre
	 * directement au dernier endroit selectionne. Si le fichier selectionne n'est pas un fichier
	 * conforme a une playlist alors le systeme nous l'indique et ne charge donc aucune playlist.
	 */
	public void loadPlaylist()
	{
		if (fileDialog == null)
			fileDialog = new FileDialog(this, "Load playlist", FileDialog.LOAD);
	...
	private void readPlaylist(String filename) throws HeadlessException, NumberFormatException
	{
		...
		if (filename.endsWith("demoPlaylist.pls"))
		{
			...
			if (filename.equals("demoPlaylist.pls"))
			{
				...
				// filename
				absolutePlaylistFilename = currentDir + File.separator + "demo" + File.separator
				         + filename;
			}
		}
	...
	/************************************************************************************************
	 * Permet de gerer une bonne partie des evenements notamment la gestion des boutons et du menu du
	 * popup
	 * 
	 * @param actionEvent
	 *           L'action event
	...
	public void actionPerformed(ActionEvent actionEvent)
	{
		...
		// Ajout d'un fichier � partir du menu
		if (actionEvent.getSource() == buttonAdd || actionCommand.equals("addfile"))
		{
			...
			{
				File[] files = fileChooser.getSelectedFiles(); // Attention gerer le
		...
		// des sous-repertoires) � partir du menu
		else
		{
			...
			// Suppression de la selection courante � partir du
			// menu du popup ou � partir du menu
			...
			// Play pour une chanson � partir du bouton de la barre de status
			// ou � partir du menu du popup
			...
			// Pause d'une chanson � partir du bouton de la barre de status
			...
			// Stop d'une chanson � partir de la barre de status
			...
			// Suppression de tous les elements de la playlist � partir
			// du menu
			...
			// Chargement d'une playlist � partir du menu
			else if (actionCommand.equals("load"))
			{
				loadPlaylist();
			}

			// Enregistrement d'une playlist � partir du menu
			...
			// Prev : passage � la chanson precedente � partir du bouton de la
			// barre
			// de status
			...
			// Play : joue une chanson � partir du menu du popup
			// Attention � rejoindre avec le play normal
			...
			// Suppression d'un fichier � partir du popup
			...
			else if (actionCommand.equals("manager"))
			{
				if (playListMgr != null)
				{
					playListMgr.setSupportedFileFormats(controller.getSupportedFileTypeExtensions());
				}

				if (playListMgr.isShowing())
					playListMgr.setVisible(false);
				else
					playListMgr.setVisible(true);
			}

			// Permet de savoir s'il faut inclure les sous repertoires �
			// l'inclusion
			// d'un repertoire � la playlist
	...
	/************************************************************************************************
	 * Ensemble des fonctions � implementer lors de l'utilisation de l'interface MouseListener
	 ***********************************************************************************************/
	public void mousePressed(MouseEvent e)
	{
	...
	public void progress(int bytesread, long microseconds, byte[] pcmdata, Map properties)
	{
		...
		if (Math.abs(progressMilliseconds - milliseconds) <= 1000)
		{
			...
			System.out.println("We do play next song ! diff = "
			         + Math.abs(progressMilliseconds - milliseconds));
			getNext();
		}
	}
