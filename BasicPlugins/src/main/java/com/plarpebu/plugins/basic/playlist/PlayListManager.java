 ...
 * @version 1.0
 */
public class PlayListManager extends JFrame implements ActionListener, KeyListener,
         DropTargetListener
{
	...
	public void keyReleased(KeyEvent e)
	{
		...
		if (o == TrackSearch)
		{
			...
			for (int i = 0; i < Tracklm.size(); i++)
			{
				...
				if (taille > 1
				         && taille <= oldTaille
				         && (sensitiveCase ? (old.indexOf(chaine) != -1) : (old.toLowerCase().indexOf(
				                  chaine) != -1)))
				{
	...
	/**
	 * ajoutes les fichiers à la playlist lors du drop nous pouvons ajouter un ou plusieurs fichier
	...
	public void parcoursRecursif(File f, boolean recu, int depth)
	{
		...
		{
			System.out.println("Stopping recursion, MAXDEPTH = " + PlayListPlugin.MAXDEPTH
			         + " reached");
			return;
		}
	...
	/**
	 * isDragOk : vérifie si le DnD est possible il est possible si on essaie de "droper" soit un
