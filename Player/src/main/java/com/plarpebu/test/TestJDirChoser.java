...
public class TestJDirChoser implements ActionListener {
  ...
  public TestJDirChoser() {
    ...
    JTextArea accessory = new JTextArea("Select directory. Use the 'Control' key" + " to select several directories");
    ...
    if(choice == JDirectoryChooser.APPROVE_OPTION) {
      ...
      JOptionPane.showMessageDialog(null, "You clicked 'Cancel'");
		...
		/*
     * JFrame f = new JFrame(); f.getContentPane().add(chooser, BorderLayout.CENTER); f.pack();
		 * f.setVisible(true);
		 */
  }
