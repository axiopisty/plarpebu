...
public class BasicMP3Player extends BasicPlayer implements BasicController, Runnable
{
	 ...
	 */
	private void initAudioInputStream(InputStream inputStream) throws UnsupportedAudioFileException,
	         IOException
	{
	...
	private void createLine() throws LineUnavailableException
	{
		...
		if (m_line == null)
		{
			...
			AudioFormat targetFormat = new AudioFormat(AudioFormat.Encoding.PCM_SIGNED, sourceFormat
			         .getSampleRate(), 16, sourceFormat.getChannels(), sourceFormat.getChannels() * 2,
	...
	public void run()
	{
		...
		synchronized (m_audioInputStream)
		{
			...
			while ((nBytesRead != -1) && (m_status != STOPPED) && (m_status != SEEKING)
			         && (m_status != UNKNOWN))
			{
				if (m_status == PLAYING)
				{
						...
						if (nBytesRead >= 0)
						{
							...
							while (it.hasNext())
							{
								...
								if (m_audioInputStream instanceof PropertiesContainer)
								{
									...
									bpl.progress(nEncodedBytes, m_line.getMicrosecondPosition(), pcm,
									         properties);
								}
								else
									bpl.progress(nEncodedBytes, m_line.getMicrosecondPosition(), pcm,
