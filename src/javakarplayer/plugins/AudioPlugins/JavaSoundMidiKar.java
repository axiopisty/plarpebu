...
import java.io.*;
import javakarplayer.plugins.AudioPlugins.taras.*;
import javakarplayer.plugins.AudioPlugins.utils.*;
...
public class JavaSoundMidiKar implements MetaEventListener {
  ...
  public JavaSoundMidiKar() {
    ...
    try {
      ...
      if (! (sequencer instanceof Synthesizer)) {
        ...
        Receiver synthRecv = (Receiver) synthesizer.getReceiver();
  ...
  public int loadFile(File file) {
    if ( (file != null) && ( ( (file.toString()).toLowerCase().endsWith(".kar")) ||
                               ( (file.toString()).toLowerCase().endsWith(".mid")))) {
      try {
        sequencer.open();
        ...
        if(!playingKar) {
          // We do not have a karaoke file, hide the kar lyrics display
  ...
  public void play() {
    if (sequencer != null) {
      ...
      // If we are playing a kar file, be sure that the JFrame for lyrics is showing
      if(playingKar) {
