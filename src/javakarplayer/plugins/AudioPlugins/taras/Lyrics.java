...
class KFrame {
  ...
  public String toString() {
    String str = "[" + start + ", " + end + "), first line = "
        + first_line + "\n";
...
public class Lyrics {
  ...
  /**
   *  Reads lyrics from MIDI sequence assuming text to be in given
   *  charset. Creates vector of syllables
   *
   *  @param seq      sequence to process
   *  @param charset  text charset
   *  @return         vector of syllables
   */
  public static Vector read(Sequence seq) throws IOException,
  ...
  private static String findCharset(Sequence seq) {
    ...
    if (!found) {
      ...
      // Most probable charset, first one is first guess etc... so the last one
      // is the best choice
   ...
   *
   * @param song      vector of syllables
   * @param type      type of events to take into account
  ...
  private static int longest_track(Vector song, int type) {
    ...
    for (int i = 0; i < song.size(); ++i) {
      ...
      if (syl.track != track) { // track change
        if (num > ltrack_num) {
          ltrack = track;
          ltrack_num = num;
        }
      ...
      if (syl.type == type && syl.text.length() > 0
          && syl.text.charAt(0) != '@')
        ++num;
    }
  ...
  /*
   * Preformats the lyrics: each syllable gets its line number, long
   * lines/words are broken and wrapped to fit into the given limit
   *
   * I  KarMaker's files have the following arrangements:
   *
   *   1. all lyrics is placed in the single track
   *   2 . song/artist/file composer name is placed as text
   *     meta-event with "@T" prefix
   *   3. language info is stored with "@L" prefix as text event
   *   4. all lyrics is text events.
   *   5. new line is started with "/"
   *   6. new paragraph is started with "\"
   *
   * II Karaoke.ru files - ?
   *   - new line may also start with "//"
   *
   *
   * @param song      text to format
   * @param cols      number of columns to use for wrapping
   */
  public static void preformat(Vector song, int cols) {
    ...
    /*
     * sift all text and leave only that from the ltrack.
     * set line numbers
     */
    int line = 0, length = 0;
    ...
    for (int i = 0; i < song.size(); ++i) {
      ...
      /*
       * we can't just skip all events in this track
       * but should check them so that they should not
       * mix with valid lyrics
       */
      if (syl.type != type) {
        if ( (syl.type != Syllable.ST_TEXT
              && syl.type != Syllable.ST_LYRICS)
            || syl.text.charAt(0) != '@')
          continue;
      }
      ...
      else if (syl.text.charAt(0) == '\\') { /* new paragraph */
        line += 2;
        syl.line = line;
        syl.text = syl.text.substring(1);
      }
      else if (syl.text.charAt(0) == '/' && syl.text.length() > 1
               && syl.text.charAt(1) == '/') {
        syl.line = ++line;
        syl.text = syl.text.substring(2);
      }
      else if (syl.text.charAt(0) == '/') { /* new line */
        syl.line = ++line;
        syl.text = syl.text.substring(1);
      }
      else {
      ...
      if (syl.type == Syllable.ST_TEXT) {
        ...
        if (length > cols) { // need to break line
   ...
   * @param song         lyrics
   * @param read_line    reading line
   * @param total_lines  overall number of lines in the frame
   */
  static void format_frame(KFrame frame, Vector song,
                           int read_line, int total_lines) {
    ...
    for (--ref_point; ref_point >= 0
         && ( (Syllable) song.elementAt(ref_point)).type != Syllable.ST_TEXT;
         --ref_point)
      ;
    ...
    for (i = ref_point; i >= 0; --i) {
      ...
      if (syl.type != Syllable.ST_TEXT)
        continue;
      if ( ( ( (Syllable) song.elementAt(ref_point)).line - syl.line)
          > read_line)
        break;
    }
    ...
    if (ref_point >= 0) {
      Syllable syl = (Syllable) song.elementAt(ref_point);
      frame.first_line = syl.line >= read_line ?
          syl.line - read_line : 0;
    }
    else {
      Syllable syl = (Syllable) song.elementAt(clear_start);
      frame.first_line = clear_start < song.size() ?
          (syl.line >= read_line ? syl.line - read_line : 0) : 0;
    }
    ...
    for (i = clear_start; i < song.size()
         && ( ( ( (Syllable) song.elementAt(i)).line - frame.first_line)
             < total_lines); ++i) {
      Syllable syl = (Syllable) song.elementAt(i);
      if (syl.type != Syllable.ST_TEXT)
        continue;
   ...
   *
   * @param   song            lyrics to format
   * @param   read_line       reading line
   * @param   total_lines     overall number of lines in the frame
   * @param   song_duration   song length in ticks
   * @return                  vector of frames
   */
  public static Vector format(Vector song, int read_line,
                              int total_lines, long song_duration) {
    long start = 0, i = 0;
   ...
   *
   * @param song      lyrics as syllables
