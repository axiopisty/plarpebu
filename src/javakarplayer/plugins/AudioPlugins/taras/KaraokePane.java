...
class KaraokePane
  ...
  AlphaComposite ac_transparent =
      AlphaComposite.getInstance(AlphaComposite.SRC_OVER, 0.5f);
  AlphaComposite ac_solid =
      AlphaComposite.getInstance(AlphaComposite.SRC);

  /**
   * Creates panel and sets size taken from properties object
   * this properties are not stored, you should pass correct
   * properties to setSong
   *
   * @param p         width and height of the panel
   */
  ...
  public KaraokePane(KaraokeProperties p) {
    ...
    addComponentListener(new ComponentAdapter() {
      public void componentResized(ComponentEvent e) {
        ...
        font = null;
        finalImage = new BufferedImage(getWidth(), getHeight(),
   ...
   *
   * @param fm  font metrics to use to measure text
  ...
  /**
   * Attempts to find font so that whole text (lines x cols) should fit
   * into current panel dimensions. The font is saved to the 'font'
   * class member. On error error is thrown
   *
  ...
  void chooseFont(Graphics g) {
    ...
    for (int size = 4; !quit; ++size) {
      ...
      if (props.height >= props.lines
          * (fm.getHeight() + fm.getLeading()) + 2 * fm.getHeight()) {
        best = size;
      }
      else {
        quit = true;
    ...
    cr_font = new Font(props.fontFace, props.style,
                       best / 3 == 0 ? 3 : best / 3);
  }

  /**
   * Attaches the given song and frames to the panel and adopts
   * given properties.
   *
   * @param s         song (vector of syllables)
   * @param f         frames (vector of frames)
  ...
  void paint(Graphics g, KFrame frame) {
    ...
    /* copyright notice */
    g.drawString(
        "Original author : Taras P. Galchenko, many improvements by M.Buffa...",
        fm.charWidth('A'), props.height - fm.getHeight() / 2);

    /* pulsation ribbon */
    double c = currentTick == -1 ? 0.6 :
        0.6 * ( (double) (frame.end - currentTick)) /
  ...
  private void repaintGradiantBackgroundImage() {
    backgroundImageGraphics.setPaint(new GradientPaint(0, 0,
        props.startGradiantColor, props.width,
        props.height, props.endGradiantColor));
    backgroundImageGraphics.fillRect(0, 0, getWidth(), getHeight());
  }

  private void drawRichText(Graphics2D g2, int x, int y, String text,
                            Color fillColor,
                            boolean drawShadow) {
    try {
      if (font == null)
      ...
      if (drawShadow) {
        if (props.displayShadow) {
          ...
          sha = textlay.getOutline(AffineTransform.getTranslateInstance(x + 4,
              y + 4));
          // transparent shadow ?
      ...
      // switch off antialiased so that only text is displayed anti-aliased !
      setAntiAliasing(g2, false);
    } catch (Exception e) {
  ...
  /**
   * Paints just the the highlighted syllables of the frame. This method
   * should be called to update visible state
   *
   * @param g         graphics context to draw in
   * @param frame     frame to draw
   */
  ...
  void paintRibbon(Graphics g, KFrame frame) {
    ...
    double c = 0.6 * ( (double) (frame.end - currentTick)) /
        ( (double) (frame.end - frame.start));
    ...
    if (props.backgroundType == KaraokeProperties.BACKGROUND_GRADIANT) {
      ...
      g.drawImage(backgroundImage, x, y, x + w, y + props.ribbonWidth,
                  (int) (sx * x), (int) (sy * y), (int) (sx * (x + w)),
                  (int) (sy * (y + props.ribbonWidth)), this);
    }
      ...
      g.drawImage(props.bgImage, x, y, x + w, y + props.ribbonWidth,
                  (int) (sx * x), (int) (sy * y), (int) (sx * (x + w)),
                  (int) (sy * (y + props.ribbonWidth)), this);
    }
  ...
  private void redrawPartialImage() {
    //finalImageGraphics.draw3DRect(damagedRectangle.x, damagedRectangle.y, damagedRectangle.width, damagedRectangle.height, true);

    //paintImmediately(damagedRectangle);
    //repaint();
    getGraphics().drawImage(finalImage,
                            damagedRectangle.x,
                            damagedRectangle.y,
                            damagedRectangle.x + damagedRectangle.width,
                            damagedRectangle.y + damagedRectangle.height,
                            damagedRectangle.x,
                            damagedRectangle.y,
                            damagedRectangle.x + damagedRectangle.width,
                            damagedRectangle.y + damagedRectangle.height,
  ...
  private void redrawAllAfterResize() {
    if (finalImageGraphics != null) {
      ...
      // paintComponent will be called after this as we have been called by a resize
      // listener
    }
  }

  /**
   * Called in response to repaint () call. Updates visible content
   * to correspond to the internal state (current song position)
   *
   * @param g         graphics context to draw in
   */
  ...
  /**
   * Called by Ticker object to signal that ribbon should be shortened.
   * If the position falls into current frame, update is requested.
   *
   * @param tick      current song position
   */
  ...
  /**
   * Called by MIDI meta event listener when meta event is encountered.
   * Frame corresponding to the given position is searched and then
   * update is requested.
   *
   * @param tick      current song position
   */
  public void seek(long tick, boolean disc) {
    ...
    if (currentFrame + 1 < frames.size()) {
      ...
      // DANGER !!!! If we do not have enough time to keep the frame rate, then
    ...
    //System.out.println("Je vais dans FULL SEARCH frame.start = " + frame.start + " " + "tick = " + tick + "frame.end = " + frame.end);
    /* full search of the frame */
    for (int i = 0; i < frames.size(); ++i) {
      frame = (KFrame) frames.elementAt(i);
      if (frame.start <= tick && tick < frame.end) {
        //System.out.println("FULL SEARCH frame.start = " + frame.start + " " + "tick = " + tick + "frame.end = " + frame.end);
        currentFrame = i;
        //repaint();
        render();
        return;
