...
public class Player
    extends JFrameWithPreferences
    implements DropTargetListener, MouseMotionListener, ComponentListener,
    Iconifiable {

      /** un tableau compose des noms des plugins que l'on veut afficher a l'origine
         et dont on souhaite leur affichage permanent**/
  ...
  /** L'unique constructeur de Player.
    Il se charge de construire l'interface principale de notre lecteur.
  ...
  public Player() throws MalformedURLException {
    ...
    plugins = (playerPlugin[]) pluginManager.getPluginInstances(pluginsSDK.
        playerPlugin.class);
    ...
    buildChargementMenu(); // simple menu pour recharcher d'eventuels nouveau plugins
  ...
  private void loadPreferences() {
    ...
    setPreferencesFileNames("preferences", "Plaperbu.properties",
                            "defaultPlaperbu.properties");
    readPreferences();
    restoreVisibility();
  }

  /**
   * Simple procedure preparant notre interface a l'operation de fermeture
   * de la fenetre principale.<br>
       * Affiche un une fenetre de dialogue pour confirmer le souhait de l'utilisateur
   * a quitter l'application.<br>
   * Ne provoque rien si l'utilisateur clique sur NON.<br>
   * Provoque la fin de l'application impliquant la sauvegarde de la derniere configuration
   * s'il clique sur OUI
   */
   ...
   * activationOrigine va permettre d'afficher des le lancement les plugins
       * correspondant au tableau prive static origine. Elle met egalement a jour la
       * liste des cases de plugins actives dans le menu. Notons que ceux-ci seront en
       * affichage permanent puisque l'on bloque ensuite la possibilite de cacher le
   * plugin.
       * L'utilisateur desirant en rajouter d'autres en affichage permanent n'aurait
   * simplement qu'a modifier dans le code le tableau origine en rajoutant la
   * String correspondant au plugin .
   * Par exemple en rajoutant origine.add("PlayList") dans le bloc static, la playliste
   * deviendrait un plugin automatiquement lance a chaque session sans avoir la
   *  possibilite de decocher la case correspondante.
   */
  private void activationOrigine() {
    ...
    // we need a handle on the playlist for possible hide/show from other plugins
    if (playlist == null)
    ...
    for (int i = 0; i < plugins.length; i++) {
      ...
      if (plugins[i].getType().equals(FramePlugin.class)) {
        // All FramePlugins will listen to the SystemExit event... so that
    ...
    for (int i = 0; i < origine.size(); i++) {
      ...
      if (! (namePlug.equals("Pan Gain") ||
             namePlug.equals("Stop Play Seek Plugin")
             || namePlug.equals("Info"))) {
  ...
  /** La fonction searchPlugin est une fonction privee a Player.
    Elle va permettre de retrouver un playerPugin en fonction de son
    nom.
  ...
  /**
   * construit le menu liee aux skins si le repertoire skins est
   * present et qu'il n'y a pas de pbs.
   */
      ...
      for (int x = 0; x < plugins.length; x++) {
        if (plugins[x].getType().equals(FramePlugin.class)) {
          ...
          if (plugins[x].getName().equals("PlayList")) {
            SwingUtilities.updateComponentTreeUI( ( (PlayListPlugin)
                plugins[x]).getToolFrame());
            SwingUtilities.updateComponentTreeUI( ( (PlayListPlugin)
                plugins[x]).getManagerFrame());
          }
        }
      }
   ...
   */
  private final JMenu loadThemes() throws IOException,
   ...
   */
  // CETTE METHODE EST A REVOIR !!! Mettre un thread qui scanne l'état des plugins ?
  private void buildPluginMenu() {
    ...
    ActionListener listener = new ActionListener() {
      public void actionPerformed(ActionEvent e) {
        ...
        if (cb.getState() == true) {
          ...
          // plugAffiche.add(pp);
          //System.out.println("Ajout ds plug affiche"+plugAffiche.size());
    ...
    if (pluginMenuItemFactory == null) {
      pluginMenuItemFactory = new PluginMenuItemFactory(menuPlugins,
      ...
      public void actionPerformed(ActionEvent e) {
        for (int i = 0; i < plugins.length; i++) {
          if (plugins[i].getType().equals(FramePlugin.class)) {
            ...
            //System.out.println("setting " + plugins[i].getName() + " to " + ((FramePlugin) plugins[i]).isVisible());
  ...
  private void buildUI() {
    ...
    StopPlaySeekPlugin listenerSPS = (StopPlaySeekPlugin) searchPlugin(
  ...
  /**
   * Fonction implémentée pour l'interface mais non
   * utile ici.
   * @param e un evenement engendré par la souris.
   */
          ...
          if (al.size() > 0) {
            ...
            while (li.hasNext()) {
              ...
              if (file != null && controller.isFileSupported(file)) {
                ...
                //reload();
                //System.out.println("on ajoute le fichier : " + file.getName());
              }
  ...
  /**
   * La fonction main : point d'entree du lecteur.
   * Elle lance le Player.
   */
  ...
  public void minimize() {
    ...
    // For the playlist plugin also (progess bar is seen through the kar cdg windows)
  ...
  /** this method is requiered because when we try to load skins from the constructor,
   * the main window is badly decorated. A bug in skinLF ?
   */
  ...
  private static String getSkinFromPreferences() {
    ...
    try {
      ...
      in = new FileInputStream("preferences" + File.separator +
                               "defaultPlaperbu.properties");
/*      System.out.println("On cherche la ressource " + "/preferences/defaultPlaperbu.properties");
      in = player.test.Player.class.getResourceAsStream("/preferences/defaultPlaperbu.properties");
*/
