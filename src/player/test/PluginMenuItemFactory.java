...
/**
 * Classe qui met dans un menu des items liées aux plugins déjà chargés.
 * Une instance de cette classe s'occupe toujours du même menu qui peut être
 * reconstruit plusieurs fois durant une session de travail. Pour être clair,
 * le JMenu est toujours le même mais ses entrées sont reconstruites à chaque
 * appel de la méthode buildMenu. Les entrées tiennent compte des plugins déjà
 * chargées au moment de l'appel de builMenu.
 * S'il y a des nouveaux plugins, de nouvelles entrées sont ajoutées au menu.
 * Si des plugins ont disparu, les entrées correspondantes ne seront plus
 * dans le menu.
 * Mais de nouvelle
 * Si les plugins ont changé, un client peut reconstruire le menu. En ce cas,
 * les anciennes entrées liées aux plugins sont enlevées et les nouvelles
 * sont ajoutées au même endroit dans le menu.
 * <P>
...
public class PluginMenuItemFactory {
  ...
  private static Logger logger =
    Logger.getLogger("player.test.PluginMenu");
   ...
   * @param menu le menu géré par cette instance.
   * @param loader le chargeur de classes des plugins.
   * @param listener l'actionDeListener qui va écouter les entrées du menu.
   */
  public PluginMenuItemFactory(JMenu menu, PluginManager loader,
   ...
   * @param type type des plugins utilisés pour construire le menu.
   * Si null, tous les types de plugin seront utilisés pour construire le menu.
  ...
  public void buildMenu(Class type) {
    ...
    logger.info("Nombre de plugins trouvés :"
