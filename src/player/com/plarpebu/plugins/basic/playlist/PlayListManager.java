...
public class PlayListManager extends JFrame implements ActionListener, KeyListener, DropTargetListener {
    ...
    public void keyReleased(KeyEvent e) {
        ...
        if (o == TrackSearch) {
            ...
            for (int i = 0; i < Tracklm.size(); i++) {
                ...
                && (sensitiveCase ? (old.indexOf(chaine) != -1) : (old.toLowerCase().indexOf(chaine) != -1))) {
    ...
    public void parcoursRecursif(File f, boolean recu, int depth) {
        ...
        for (int i = 0; i < tab.length; i++) {
            ...
            if ((tab[i].isFile() && path.endsWith(".mp3")) || (tab[i].isFile() && path.endsWith(".kar"))
            || (tab[i].isFile() && path.endsWith(".mid")) || (tab[i].isFile() && path.endsWith(".zip"))) {
