...
public class PlayListPlugin
  ...
  private JToggleButton buttonShuffle;
  private JToggleButton buttonRepeat;
  private JCheckBox cb;
  private JLabel time;
  private JPanel p, pnl;
  private JMenuBar menuBar;
  private JMenu menu;
  private JMenuItem menuItem;
  private ToolTipManager tpm;
  private FileDialog fdplaylist;
  ...
  public PlayListPlugin() {
    ...
    //lstPlayList.setTransferHandler(this);
    /** Drag and drop */
    DropTarget dt = new DropTarget(lstPlayList,
  ...
  private void loadPreferences() throws NumberFormatException,
      HeadlessException {
    // inherited behavior
    readPreferences();
    ...
    try {
      ...
      //System.out.println("Lu dans les prefs currentPlaylist = " + currentPlaylistFilename);
      ...
      c = StringToColor(preferences.getProperty(
  ...
  public void setPlayListFont(String font, int fontSize, int fontStyle) {
    lstPlayList.setFont(new Font(font, fontStyle, fontSize));
    fontName = font;
    fontSize = fontSize;
    fontStyle = fontStyle;
  ...
  /** Permet de creer un nouveau bouton en lui associant, une action,
   ...
   * @param imageName le chemin d'accés a l'icone
   */
  JButton addButton(JComponent p, String name, String tooltiptext,
                    String imageName) {
    JButton b;
   ...
   * @param imageName le chemin d'accés a l'icone
   */
  JToggleButton addToggleButton(JComponent p, String name, String tooltiptext,
                                String imageName) {
    JToggleButton b;
    if ( (imageName == null) || (imageName.equals(""))) {
  ...
  public static void getNext() {
    paused = false;
    /*if (shuffle) {
      int newVal = getIndexShuffle();
      lstPlayList.setSelectedIndex(newVal);
          }
          else {*/
   ...
   * actuelle en mode texte [lisible donc par l'homme ].
       * Les 3 premieres lignes indiqueront la fonte , taille et style de la playlist
       * (au niveau ecriture) ensuite les lignes suivantes seront composees du titre
   * des chansons avec son numero de lecture.
   * Notons que l'interface de dialogue avec les fichiers conservent le dernier
   * choix (repertoire + fichier) pour pouvoir a une prochaine ouverture de
   * la boite de dialogue etre directement au dernier endroit selectionne.
   */
  public void savePlaylist() {
    FileDialog fd = new FileDialog(this, "Save playlist",
                                   FileDialog.SAVE);
   ...
   * choix (repertoire + fichier) pour pouvoir a une prochaine ouverture de
   * la boite de dialogue etre directement au dernier endroit selectionne.
   * Si le fichier selectionne n'est pas un fichier conforme a une playlist
   * alors le systeme nous l'indique et ne charge donc aucune playlist.
   */
  public void loadPlaylist() {

    if (fd == null)
      fd = new FileDialog(this, "Load playlist",
                          FileDialog.LOAD);
    fd.setDirectory(lastDirSelected);
    fd.setFile(lastFileSelected);
    fd.show();
  ...
  private void readPlaylist(String filename) throws HeadlessException,
    ...
    if (filename.endsWith("demoPlaylist.pls")) {
      ...
      // the filename has been read from the defaultPlaylistPlugin.properties file
      // In that case, we dont't know where the software has been installed and we cannot use
      // an absolute filename.
      // If the filename is not equal but ends with "demoPlaylist.pls" then it is the case
      // when the user lauched the software for the first time, opened the default demo playlist
      // and then quit. Preferences have been saved and the default playlist is an absolute
      // filename. But in that case, the songs are relative... this is why we have to test the two cases...
      if (filename.equals("demoPlaylist.pls")) {
        // This is a relative filename, add the current dir before the filename
        absolutePlaylistFilename = currentDir + File.separator + "preferences" +
    ...
    if (!f.exists()) {
      System.out.println("file " + absolutePlaylistFilename +
                         " does not exists");
    }
  ...
  public void parcoursRecursif(File f, boolean recu, int depth) {
    if (depth > MAXDEPTH) {
      System.out.println("Stopping recursion, MAXDEPTH = " + MAXDEPTH +
                         " reached");
      return;
    }
    ...
    for (int i = 0; i < tab.length; i++) {
      ...
      if (tab[i].isFile() && controller.isFileSupported(path)) {
        ...
        if (! (tab[i].getName().equals("System Volume Information")) &&
            recu) {
  ...
  /** Permet de gerer une bonne partie des evenements
   * notamment la gestion des boutons et du menu du popup
  ...
  public void actionPerformed(ActionEvent e) {
    ...
    if (e.getSource() == buttonAdd || c.equals("addfile")) {
      ...
      if (returnVal == JFileChooser.APPROVE_OPTION) {
        File[] files = fc.getSelectedFiles(); // Attention gerer le cas ou il ne renvoie rien
        //
    ...
    // des sous-repertoires) à partir du menu
      ...
      // ou à partir du menu du popup
        ...
        try {
          if (listModel.size() != 0 && ! (lstPlayList.isSelectionEmpty())) {
            ...
            try {
              MpegInfo mi = new MpegInfo( ( (File) listModel.elementAt(index)).
                                         getPath());
      ...
      // Next : passage à la chanson suivante à partir du bouton de la barre
      // de status
      else if (e.getSource() == buttonNext) {
        getNext();
      }

      // Prev : passage à la chanson precedente à partir du bouton de la barre
      // de status
      ...
      // Permet de savoir s'il faut inclure les sous repertoires à l'inclusion
  ...
  class PopupListener
    ...
    /** Fonction qui initialise le PopUp lors de son affichage
        i.e. qui active ou désactive certains menus selon le contexte */
  ...
  /** Ensemble des fonctions à implementer lors de l'utilisation
  ...
  public void progress(int bytesread, long microseconds, byte[] pcmdata,
    ...
    //if (millisec_to_time(progressMilliseconds).equals(millisec_to_time(
      //  milliseconds))) {
  ...
  /**
   * ajoutes les fichiers à la playlist lors du drop
   * nous pouvons ajouter un ou plusieurs fichier venant de l'exterieur ainsi
   * que des playlist
      ...
      try {
        ...
        if (data instanceof java.util.List) {
          ...
          if (al.size() > 0) {
            ...
            /** ajout des données recues dans la liste graphique et réelle */
  ...
  /**
   * isDragOk : vérifie si le DnD est possible
   * il est possible si on essaie de "droper" soit un fichier soit une String
   * @param e DropTargetDragEvent
   * @return boolean
   */
  private void addDir() {
