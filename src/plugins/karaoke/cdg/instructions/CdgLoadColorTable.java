 ...
 * Load Color Table High [colors 8 thru 15] (subCode.instruction==31)
 ...
 in java a short is 2 bytes, so the data field must be processed 2 bytes 2 bytes
...
public abstract class CdgLoadColorTable {
   ...
   * startIndex, which can be 0 or 8 depending on the case we
   * are processing a LoadColorTableLow or a LoadColorTableHigh
   * cdg instruction
   * @param data : the 16 bits data chunk that encode 8 colors
   * @param startIndex : 0 or 8 (low/high colormap)
   * @param colormap : the colormap that is going to be half-filled
   */
  public static void setColormap(byte[] data, int startIndex, Color []colormap) {
    ...
    for (int i = 0; i < 16; i += 2) {
      ...
      // blue, green and red are color value in the range [0-15] as they are
      // coded in 4 bits. Let's turn them into "real" modern colors in the
      // range [0-255]. Let's multiply the [0-15] value by 255, and divide by 15
      // i.e multiply by 17 !
  ...
  public static void setColormapElement(Color []colormap, int startIndex, int index, int red, int green, int blue) {
    //System.out.println("SetColormapElement LOW index=" + (index/2) +" red = " + red + " green = " + green + " blue = " +blue);
