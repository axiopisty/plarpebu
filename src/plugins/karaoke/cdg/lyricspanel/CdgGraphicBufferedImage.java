package plugins.karaoke.cdg.lyricspanel;
...
/**
 * <p>Title: </p>
 * <p>Description: In the CD+G system, 16 color graphics are displayed on a
 * raster field which is 300 x 216 pixels in size.  The middle 294 x 204 area
     * is within the TV's "safe area", and that is where the graphics are displayed.
 * The outer border is set to a solid color.  The colors are stored in a
 * 16 entry color lookup table.
 </p>
...
public class CdgGraphicBufferedImage
    extends JPanel {
  ...
  private void redrawPartialImage(Rectangle r) {
    if (forceRedrawFullImage) {
      ...
      // source rectangle : zone in the pixels byte array
      this.r = r;
      //r = new Rectangle(r.x, r.y, (int)Math.round(1.1*r.width), (int)Math.round(1.1*r.height));
...
*/
    if(fromPartialRedraw) {
      g.drawImage(img,
                  rDestination.x,
                  rDestination.y,
                  rDestination.x + rDestination.width,
                  rDestination.y + rDestination.height,
                  r.x,
                  r.y,
                  r.x + r.width,
                  r.y + r.height,
