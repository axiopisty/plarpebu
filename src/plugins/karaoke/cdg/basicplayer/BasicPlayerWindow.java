...
import plugins.karaoke.cdg.basicplayer.gui.*;
import plugins.karaoke.cdg.instructions.*;
import plugins.karaoke.cdg.io.*;
import plugins.karaoke.cdg.lyricspanel.*;
import pluginsSDK.*;
...
public class BasicPlayerWindow
    extends FramePlugin
  ...
  private void loadPreferences() throws NumberFormatException,
    ...
    try {
      ...
      if ( (fsWidth != -1) && (fsHeight != -1) && (fsDepth != -1) &&
          (fsFreq != -1)) {
        System.out.println("Read prefs for fs mode, plug " + getName() + " : (" +
                           fsWidth + "," + fsHeight + "," + fsDepth + "," +
                           fsFreq + ")");
   ...
   */
  /*  public void play(File file, int nbCdgInstructions) {
      try {
        if (player == null) {
          player = new BasicPlayer(this);
        }
        System.out.println("Opening mp3 file : " + file.getAbsolutePath());
        player.setDataSource(new File(file.getAbsolutePath()));
        //player.setDataSource(new File(file.getAbsolutePath()));
        loadCdgFile(file);
        cdgFileLoaded = true;
   ...
   */
  /*  public void playCdgOnly(int nbCdgInstructions) {
      if (cdgFileLoaded)
        startTimedPlay();
    }
   */
   ...
   * @param mp3File : the name of the mp3File, its basename will be
   * used for getting the cdg file associated
   */
  public void loadCdgFile(File mp3File) {
    try {
      int length = mp3File.getAbsolutePath().length();
      String cdgFileName = mp3File.getAbsolutePath().substring(0, length - 4) +
          ".cdg";
  ...
  private void startTimedPlay() throws
    ...
    System.out.println("---We launch timer with delay = " +
                       (nbCdgInstructions * 3.33) + "---");
    ...
    timer = new Timer(delay, new ActionListener() {
      ...
      public void actionPerformed(ActionEvent e) {
        ...
        if (!pausedPlay) {
          ...
          // After we played the buffer of instructions, do the "asservissement"
          if (tempsMp3 != 0) {
            if ( (current_row * 3.33) > (tempsMp3 + 100)) {
              // we check if previously we were speeding up...
              if (timer.getDelay() < delay) {
                // we were speeding up, let's go back to the "normal" value
                timer.setDelay(delay);
                //jLabel1.setText("standard delay\t\t " + (int)(current_row * 3.33) + "\t>\t" + tempsMp3 + "\t\tdelay : " + timer.getDelay());
              }
              else {
                // let's continue slowing down
                //System.out.println("ralentit " + (current_row * 3.33) + " > " + tempsMp3);
                timer.setDelay(timer.getDelay() + 100);
                //jLabel1.setText("slowing down\t\t " + (int)(current_row * 3.33) + "\t>\t" + tempsMp3 + "\t\tdelay : " + timer.getDelay());
              }
              ...
              if (timer.getDelay() > delay) {
                // we were slowing down, let's go back to the "normal" value
                timer.setDelay(delay);
                //jLabel1.setText("standard delay\t\t " + (int)(current_row * 3.33) + "\t>\t" + tempsMp3 + "\t\tdelay : " + timer.getDelay());
              }
              else {

                //System.out.println("accélère " + (current_row * 3.33) + " < " + tempsMp3);
                if (timer.getDelay() > 0) {
  ...
  public void playCdgInstruction() {
    ...
    if (cdgDataChunksArray[current_row].getCdgInstruction() ==
        CdgDataChunk.CDG_LOAD_COL_TABLE_LOW) {
      // Allocate the colors 0-7 of the colormap
      CdgLoadColorTable.setColormap(cdgDataChunksArray[current_row].getCdgData(),
                                    0,
                                    colormap);
      //panelColormap.setColormap(colormap);
      panelLyrics.setColormapLow(colormap);
    }
    else if (cdgDataChunksArray[current_row].getCdgInstruction() ==
             CdgDataChunk.CDG_LOAD_COL_TABLE_HIGH) {
      ...
      panelLyrics.setColormapHigh(colormap);
             ...
             CdgDataChunk.CDG_TILE_NORMAL) {
      damagedRectangle = CdgTileBlock.drawTile(cdgDataChunksArray[current_row].
                                               getCdgData(),
                                               panelLyrics.getPixels(), false);
      panelLyrics.pixelsChanged(damagedRectangle);
    }
    else if (cdgDataChunksArray[current_row].getCdgInstruction() ==
             CdgDataChunk.CDG_TILE_XOR) {
      damagedRectangle = CdgTileBlock.drawTile(cdgDataChunksArray[current_row].
                                               getCdgData(),
      ...
      panelLyrics.pixelsChanged(damagedRectangle);
             ...
             CdgDataChunk.CDG_MEMORY_PRESET) {
      if (CdgMemoryPreset.clearScreen(cdgDataChunksArray[current_row].
                                      getCdgData(),
                                      panelLyrics.getPixels())) {
        // if previous instructions returned false, the screen has already
        // been cleared in a previous call...
        panelLyrics.pixelsChanged();
      }
    }
    else if(cdgDataChunksArray[current_row].getCdgInstruction() ==
             CdgDataChunk.CDG_BORDER_PRESET) {
       CdgBorderPreset.drawBorder(cdgDataChunksArray[current_row].getCdgData(),
                                  panelLyrics.getPixels());

      panelLyrics.pixelsChanged();
    } else if(cdgDataChunksArray[current_row].getCdgInstruction() ==
             CdgDataChunk.CDG_SCROLL_COPY) {
     panelLyrics.savePixels();
     ret = CdgScrollCopy.scroll(cdgDataChunksArray[current_row].getCdgData(),
                          panelLyrics.getPixels());

     panelLyrics.pixelsChanged();
     if (!ret)panelLyrics.restorePixels();
   } else if(cdgDataChunksArray[current_row].getCdgInstruction() ==
             CdgDataChunk.CDG_SCROLL_PRESET) {
     panelLyrics.savePixels();
     ret = CdgScrollPreset.scroll(cdgDataChunksArray[current_row].getCdgData(),
                          panelLyrics.getPixels());
  ...
  private void jbInit() throws Exception {
    ...
    jMenuItemFullScreensOptions.addActionListener(new
        BasicPlayerWindow_jMenuItemFullScreensOptions_actionAdapter(this));
    jMenuItemLyricsSyncOptions.setText("Lyrics not in sinc ?");
    jMenuItemLyricsSyncOptions.addActionListener(new
        BasicPlayerWindow_jMenuItemLyricsSyncOptions_actionAdapter(this));
    jMenuItemFullScreenWindow.setText("Fullscreen/Window");
    jMenuItemFullScreenWindow.addActionListener(new BasicPlayerWindow_jMenuItemFullScreenWindow_actionAdapter(this));
  ...
  void jMenuItemLyricsSyncOptions_actionPerformed(ActionEvent e) {
    ...
    panelLyrics.redrawFullImage();
    preferences.setProperty("redrawAllFrame",
                            "" + "" + cdgOptionsDialog.getRedrawFullImage());
  }
...
class BasicPlayerWindow_jMenuItemFullScreensOptions_actionAdapter
    implements java.awt.event.ActionListener {
  BasicPlayerWindow adaptee;

  BasicPlayerWindow_jMenuItemFullScreensOptions_actionAdapter(BasicPlayerWindow
      adaptee) {
...
class BasicPlayerWindow_jMenuItemLyricsSyncOptions_actionAdapter
    implements java.awt.event.ActionListener {
  BasicPlayerWindow adaptee;

  BasicPlayerWindow_jMenuItemLyricsSyncOptions_actionAdapter(BasicPlayerWindow
      adaptee) {
    this.adaptee = adaptee;
  }
...
class BasicPlayerWindow_jMenuItemFullScreenWindow_actionAdapter
    implements java.awt.event.ActionListener {
  BasicPlayerWindow adaptee;
