package plugins.karaoke.cdg.io;
 ...
 * </p>
 * <p>
 * Description: this class describe a typical cdg data chunk of 1+1+2+16+4 = 24
 * bytes, as stated in the philipps red book. A CDG file is a list of chunks of
 * that type.
 * </p>
 ...
 * commands below. When reading the command and instruction fields, you should
 * only pay attention to the lower 6 bits of each of these bytes (e.g. by ANDing
 * the byte with the hex value 3Fh). The upper 2 bits are used for the P and Q
 * channels which you should ignore. If the lower 6 bits of the command field
 * are equal to 9, then the SubCode packet contains CD+G information, otherwise
 * it should be ignored. If the packet contains CD+G information, then you use
 * the instruction field (again, the lower six bits), to determine how to
 * interpret the data. An example will be shown below.
 * <p>
...
public class CdgDataChunk {
    ...
    public String toString() {
        ...
        if (cdgCommand == 9) {
            ...
            else if (cdgInstruction == CDG_TILE_XOR) description += "Tile Block(xor)";
        }
