 ...
 * Larger images are built by using multiple tiles. The XOR variant is a special
 * case of the same command, the difference is described below. The tile is
 * stored using 1-bit graphics. The structure contains two colors which are to
 * be used when rendering the tile. The tile is extracted from 16 bytes of
 * subCode.data[] in the following manner: byte color0; // Only lower 4 bits are
 * used, mask with 0x0F byte color1; // Only lower 4 bits are used, mask with
 * 0x0F byte row; // Only lower 5 bits are used, mask with 0x1F byte column; //
 * Only lower 6 bits are used, mask with 0x3F byte tilePixels[12]; // Only lower
 * 6 bits of each byte are used color0, color1 describe the two colors (from the
 * color table) which are to be used when rendering the tile. Color0 is used for
 * 0 bits, Color1 is used for 1 bits. Row and Column describe the position of
 * the tile in tile coordinate space. To convert to pixels, multiply row by 12,
 * and column by 6. tilePixels[] contains the actual bit values for the tile,
 * six pixels per byte. The uppermost valid bit of each byte (0x20) contains the
 * left-most pixel of each scanline of the tile. In the normal instruction, the
 * corresponding colors from the color table are simply copied to the screen. In
 * the XOR variant, the color values are combined with the color values that are
 * already onscreen using the XOR operator. Since CD+G only allows a maximum of
