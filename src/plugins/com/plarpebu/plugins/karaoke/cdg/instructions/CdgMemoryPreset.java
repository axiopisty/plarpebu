 ...
 * </p>
 * <p>
 * Description: Memory Filler instruction In this instruction, the 16 byte data
 * field is interepreted as follows. byte color; // Only lower 4 bits are used,
 * mask with 0x0F byte repeat; // Only lower 4 bits are used, mask with 0x0F
 * byte filler[14]; When these commands appear in bunches (to insure that the
 * screen gets cleared), the repeat count is used to number them. If this is
 * true, and you have a reliable data stream, you can ignore the command if
 * repeat != 0. Note that in all these instructions, only the lower 6 bits of
 * each data byte are used. So, for example, If I refer to subCode.data[0], I
 * actually mean (subCode.data[0] & 0x3F).
 * <p>
