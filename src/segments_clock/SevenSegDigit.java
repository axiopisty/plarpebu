...
public class SevenSegDigit
{
    ...
    public SevenSegDigit()
    {
        ...
        SegmentMap = (new byte[] {
            DISPLAY_SEGMENTS_0, DISPLAY_SEGMENTS_1, DISPLAY_SEGMENTS_2, DISPLAY_SEGMENTS_3, DISPLAY_SEGMENTS_4, DISPLAY_SEGMENTS_5, DISPLAY_SEGMENTS_6, DISPLAY_SEGMENTS_7, DISPLAY_SEGMENTS_8, DISPLAY_SEGMENTS_9
        });
        ...
        // DP1 and DP2 are arrays for displaying ":" By default, display ":" between hours and minutes,
    ...
    void CalcSegments(double d, int i, int j, double d1, int k)
    {
        ...
        for(int l = 0; l < 7; l++)
        {
            ...
            {
                Segments[l][i1].x = (int)(((double)TempSegments[l][i1].x + (double)Thicks[l][i1].x * d4 + ((double)TempSegments[l][i1].y + (double)Thicks[l][i1].y * d4) * d3) * d1 + (double)i);
    ...
    public void PaintSevenSegDisplay(Graphics g, boolean flag, Component component)
    {
        ...
        double d2 = 7D;
        byte byte0 = 10;
        if(Xsize != PrevXsize || Ysize != PrevYsize || NumDigits != PrevNumDigits)
            flag = false;
        PrevXsize = Xsize;
        PrevYsize = Ysize;
  ...
  public void setNumDigits(int NumDigits) {
    ...
    static
    {
        ...
        GapG = new Point[7];
        Gaps = (new Point[][] {
            GapA, GapB, GapC, GapD, GapE, GapF, GapG
        });
        Thicks = (new Point[][] {
            ThickA, ThickB, ThickC, ThickD, ThickE, ThickF, ThickG
        });
        TempSegments = (new Point[][] {
            TempSegmentA, TempSegmentB, TempSegmentC, TempSegmentD, TempSegmentE, TempSegmentF, TempSegmentG
        });
        SEGMENT_A = 1;
        SEGMENT_B = 2;
        SEGMENT_C = 4;
